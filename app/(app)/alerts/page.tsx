...
import { canManageContract, canMutate } from "@/lib/permissions";
...
export default async function AlertsPage({
  ...
  const allVendorsWithCompliance = (vendorsWithCompliance as Vendor[] | null) ?? [];
  const vendorsNeedingAttention = allVendorsWithCompliance.filter(
    ...
    <div className="space-y-8">
      ...
      <section className="space-y-3">
        <div>
          ...
          <p className="mt-1 text-sm text-slate-500">
            A vendor&apos;s own compliance paperwork (e.g. a master certificate of insurance) — tied to the
            vendor relationship itself, not one contract.
          </p>
        </div>

        {vendorsNeedingAttention.length === 0 && (
          <div className={`${panel} p-8 text-center text-slate-500`}>No vendor compliance documents need attention.</div>
        )}
              ...
              <table className="min-w-full text-sm">
                ...
                <tbody>
                  {vendorsNeedingAttention.map((vendor) => {
                    const expired = isPastEndDate(vendor.compliance_doc_expires_on);
                    const acknowledge = acknowledgeVendorComplianceExpiry.bind(null, vendor.id);
                    return (
                      <tr key={vendor.id} className={`${tr} ${expired ? "bg-red-50" : "bg-orange-50"}`}>
                        <td className="px-4 py-3">
                          <Link href={`/vendors/${vendor.id}`} className="font-medium text-slate-900 hover:text-blue-700">
                            {vendor.name}
                          </Link>
                          <span className={`ml-2 ${code}`}>{vendor.vendor_code}</span>
                        </td>
                        <td className={`${td} capitalize`}>{vendor.risk_tier ?? "Not rated"}</td>
                        <td className={td}>
                          <div className="flex items-center gap-2">
                            {formatDate(vendor.compliance_doc_expires_on)}
                            <span
                              className={`rounded-full px-2 py-0.5 text-[11px] font-bold uppercase tracking-wide text-white ${
                                expired ? "bg-red-600" : "bg-orange-500"
                              }`}
                        ...
                        <td className="px-4 py-3 text-right">
                          <div className="flex flex-col items-stretch gap-2 sm:flex-row sm:items-center sm:justify-end">
                            <Link href={`/vendors/${vendor.id}`} className={`${secondaryButton} text-center`}>
                              View vendor
                            </Link>
                            {canEdit && (
                              <form action={acknowledge}>
                                <ConfirmSubmitButton
                                  confirmMessage="Acknowledge this vendor's compliance document alert? It'll drop off this list until the situation changes."
                                  className={`${primaryButton} w-full`}
                                >
                                  Acknowledge
                                </ConfirmSubmitButton>
        ...
        {acknowledgedVendors.length > 0 && (
          <details className={`p-6 ${panel}`}>
            <summary className="cursor-pointer text-[11px] font-bold uppercase tracking-wider text-slate-500">
              Acknowledged ({acknowledgedVendors.length})
            </summary>
            <div className={`${panelHeader} mt-4 rounded-md`}>
              <ul className="divide-y divide-slate-200">
                {acknowledgedVendors.map((vendor) => (
                  <li key={vendor.id} className="flex flex-col gap-0.5 px-2 py-2 text-sm sm:flex-row sm:items-center sm:justify-between">
                    <Link href={`/vendors/${vendor.id}`} className="font-medium text-slate-900 hover:text-blue-700">
                      {vendor.name}
                    </Link>
